...
do 
    ...
    CUDA_VISIBLE_DEVICES=$device_id python3 $PROJECT_PATH/thumt/bin/translator.py \
        ...
        --output $OUTPUT_PATH/$domain.$type.knnmt.lpv$lpv.k$K.lambda$LAMBDA.temp$TEMP.output  \
    ...
    data_input=$OUTPUT_PATH/$domain.$type.knnmt.lpv$lpv.k$K.lambda$LAMBDA.temp$TEMP.output
