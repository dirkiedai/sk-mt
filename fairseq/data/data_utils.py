...
def collate_tokens_2d(
    ...
    """Added by dirkiedai"""
    size_1 = max(v.size(0) for v in values)
    size_2 = max(v.size(1) for v in values)
    size_1 = size_1 if pad_to_length is None else max(size_1, pad_to_length)
    size_2 = size_2 if pad_to_length is None else max(size_2, pad_to_length)

    if pad_to_multiple != 1 and size % pad_to_multiple != 0:
        size = int(((size - 0.1) // pad_to_multiple + 1) * pad_to_multiple)
    res = values[0].new(len(values), size_1, size_2).fill_(pad_idx)


    def copy_tensor(src, dst):
        assert dst.numel() == src.numel()
        if move_eos_to_beginning:
            src[src.eq(eos_idx)] = pad_idx
            if eos_idx is None:
                # if no eos_idx is specified, then use the last token in src
                dst[:, 0] = src[:, -1]
            else:
    ...
    for i, v in enumerate(values):
        ...
        copy_tensor(v, res[i, :v.size(0), size_2 - v.size(1) :] if left_pad else res[i, :v.size(0), :v.size(1)])
    return res
