...
class KNN_Dstore(object):
    ...
    def __init__(self, args, trg_vocab_size):
        ...
        self.load_knn_datastore = args.load_knn_datastore
    ...
    def reset(self):
        ...
        self.vals = torch.from_numpy(self.vals).to('cuda' if self.use_gpu_to_search else 'cpu')



    def add_entry(self, features, target):
        ...
        if self.use_gpu_to_search:
            self.vals[self.dstore_idx:reduce_size + self.dstore_idx] = target.int()
        else:
            self.vals[self.dstore_idx:reduce_size + self.dstore_idx] = target.cpu().int()
        self.dstore_idx += reduce_size

        if not self.load_knn_datastore:
            self.index.add(features.cuda() if self.use_gpu_to_search else features.cpu())
    ...
    def setup_faiss(self, args):
        ...
        if args.load_knn_datastore:
            ...
            faiss_index_path = args.faiss_index if args.faiss_index else args.dstore_filename + '/knn_index'
    ...
    def retrieve(self, queries, padding = None):
        ...
        tgt_idx = self.vals[knns.to(self.vals.device)].to(queries.device).squeeze(-1)  # [Batch size * Seq len, K]
...
if __name__ == "__main__":
    class ARGS:
        ...
        knn_temperature = 1.0
        indexfile = ''
        dstore_filename = 'interactive-mt'
        no_load_keys = False
        probe = 32
