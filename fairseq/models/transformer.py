...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--knn-lambda-temperature-value", default=10, type=float)
        ...
        parser.add_argument("--use-faiss-to-search", default=False, action="store_true")
    @classmethod
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if args.use_knn_datastore:
            self.knn_datastore = KNN_Dstore(args, self.vocab_size)

        self.use_knn_datastore = args.use_knn_datastore
        self.knn_lambda_type = args.knn_lambda_type
        self.knn_lambda_value = args.knn_lambda_value
        self.knn_temperature_type = args.knn_temperature_type
        self.knn_temperature_value = args.knn_temperature_value

        self.knn_lambda_temperature_value = args.knn_lambda_temperature_value
        self.knn_k_type = args.knn_k_type
        self.knn_k_value = args.k
    ...
    def forward(
        ...
        if self.use_knn_datastore:
            ...
            if isinstance(self.knn_datastore, KNN_Dstore):
            # we should return the prob of knn search
                ...
                knn_prob_lst, knn_dists_lst, knn_index_lst = [], [], []

                if self.args.use_faiss_to_search:
                    for i in range(bsz):
                    # we should return the prob of knn search
                        query = last_hidden[i].unsqueeze(0)
                        knn_search_result = self.knn_datastore[i].retrieve(query)
                        # knn_probs = knn_search_result['prob']
                        knn_dists = knn_search_result['distance']  # [batch, seq len, k]  # we need do sort
                        knn_index = knn_search_result['knn_index']
                        tgt_index = knn_search_result['tgt_index']
                        knn_temperature = self.knn_datastore[i].get_temperature() 
                        decode_result = self.knn_datastore[i].calculate_knn_prob(knn_index, tgt_index, knn_dists, query,
                                                                                knn_temperature)
                        
                        knn_dists_lst.append(knn_dists)
                        knn_index_lst.append(knn_index)
                        knn_prob_lst.append(decode_result['prob'])
                    
                    knn_prob = torch.cat(knn_prob_lst)
                    knn_dists = torch.cat(knn_dists_lst)
                    knn_index = torch.cat(knn_index_lst)
                    knn_lambda = torch.nn.functional.relu(1.0 - knn_dists[:,:,0] / self.knn_temperature_value)
                else:

                    query = last_hidden
                    bsz, seq_len = query.size(0), query.size(1)

                    keys, vals, masks = self.knn_datastore

                    keys_mask = ((1.0 - masks.float()) * -1e4).to(keys).reshape([bsz, -1])

                    dists = ((keys - query) ** 2).sum(-1)
                    scaled_dists = -1.0 / self.knn_temperature_value * dists + keys_mask

                    knn_dists, knn_index = torch.topk(scaled_dists, 
                        self.knn_k_value if keys.size(0) >= self.knn_k_value else keys.size(0))

                    tgt_index = torch.gather(vals.squeeze(-1), 1, knn_index)

                    knn_weight = torch.softmax(knn_dists, dim=-1).unsqueeze(-1)

                    knn_prob = torch.zeros([bsz, self.knn_k_value if keys.size(1) > self.knn_k_value else keys.size(1), self.vocab_size],
                    dtype=torch.float32, device=keys.device)

                    scatter(src=knn_weight.float(), out = knn_prob, index = tgt_index, dim = -1)

                    knn_prob = knn_prob.sum(dim=-2).half() if self.args.dstore_fp16 else knn_prob.sum(dim=-2)

                    knn_prob = knn_prob.unsqueeze(1)

                    knn_lambda = torch.nn.functional.relu(1.0 + knn_dists[:,0]).unsqueeze(-1).unsqueeze(-1)
                    # knn_lambda = torch.exp(knn_dists[:,0]).unsqueeze(-1).unsqueeze(-1)
                    ...
                    #     keys, vals = self.knn_datastore[i]

                    #     dists = ((keys - query) ** 2).sum(-1)
                    #     scaled_dists = -1.0 / self.knn_temperature_value * dists

                        # knn_dists, knn_index = torch.topk(scaled_dists, 
                        # self.knn_k_value if keys.size(0) >= self.knn_k_value else keys.size(0))

                    #     tgt_index = vals[knn_index.to(vals.device)].to(query.device).squeeze(-1)

                    #     knn_weight = torch.softmax(knn_dists, dim=-1).unsqueeze(-1)

                    #     prob = torch.zeros(seq_len, self.vocab_size, device = query.device) #[B, S, Vocab Size]
            
                    #     scatter(src=knn_weight.float().squeeze(-1), out = prob, index = tgt_index, dim = -1)
                        
                    #     knn_dists_lst.append(knn_dists.unsqueeze(0))
                    #     knn_index_lst.append(knn_index.unsqueeze(0))
                    #     knn_prob_lst.append(prob.unsqueeze(0))

                    # knn_prob = torch.cat(knn_prob_lst)
                    # knn_dists = torch.cat(knn_dists_lst)
                    # knn_index = torch.cat(knn_index_lst)

                    # knn_lambda = torch.nn.functional.relu(1.0 + knn_dists[:,:,0])
    ...
    def get_normalized_probs(
        ...
        if hasattr(self, "adaptive_softmax") and self.adaptive_softmax is not None:
            if sample is not None:
                ...
                target = None
            network_probs = self.adaptive_softmax.get_log_prob(logits, target=target).exp_()
        ...
        if self.use_knn_datastore:
            ...
            if self.knn_lambda_type == "fix":
                knn_probs = network_probs * (1 - knn_lambda) + knn_probs * knn_lambda
            else:
                knn_probs = network_probs * (1 - knn_lambda) + knn_probs

            if log_probs:
                return torch.log(knn_probs)
            else:
                return knn_probs
                
        return network_probs if not log_probs else torch.log(network_probs)
