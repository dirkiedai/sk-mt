...
def _main(args, output_file):
    ...
    while(round < args.nround):
        ...
        for sample in progress:
            ...
            sample["net_input"]["prev_output_tokens"] = utils.strip_pad(sample["net_input"]["prev_output_tokens"], tgt_dict.unk()).reshape(1,-1)
            ...
            if(record["tgt len"] > 2 * record["src len"] or record['tgt len'] < 0.5 * record['src len']):
                continue
            if(record['src len'] < 10 or record['src len'] > 100):
                continue
            ...
            if args.prefix_constrained:
                ...
                if args.unicost:
                    ...
                    deletes, inserts, substitutes = 1, 1, 2
                ...
                while(not refine_stop):
                    ...
                    for j, hypo in enumerate(hypos[0][: args.nbest]):
                        ...
                        if debpe_hypo_str == target_str:
                            ...
                            if args.quick_add_datastore:
                                model.decoder.knn_datastore.add_entry(_model_args, features[last_prefix_size:, :], target[last_prefix_size: ])
                            refine_stop = 1
                            operation = None
                        ...
                        if generate:
                            ...
                            if args.quick_add_datastore:
                                model.decoder.knn_datastore.add_entry(_model_args, features[last_prefix_size:prefix_size, :], target[last_prefix_size: prefix_size])
                ...
                if not args.quick_add_datastore:
                    model.decoder.knn_datastore.add_entry(_model_args, features, target)
                if args.clear_datastore:
                    model.decoder.knn_datastore.reset()
