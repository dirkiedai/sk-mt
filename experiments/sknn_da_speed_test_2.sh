...
do
    ...
    if test -f $OUTPUT_PATH/sknn-bsz$BATCH_SIZE-generate.txt
    then rm -f $OUTPUT_PATH/sknn-bsz$BATCH_SIZE-generate.txt
    fi
    ...
    CUDA_VISIBLE_DEVICES=1 python3 /data/dirkiedye/knn-mt-research/sknn-mt/experimental_generate.py $DATA_PATH \
        ...
        | tee -a "$OUTPUT_PATH"/sknn-bsz$BATCH_SIZE-generate.txt
    done
done